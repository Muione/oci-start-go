...
// UpdateInstanceShape modifies the shape (and optionally OCPU/memory/displayName)
// of a running or stopped instance. The instance must be STOPPED for most shape
// changes; some flexible shapes support in-place resizing without stop.
func UpdateInstanceShape(ctx context.Context, c Clients, instanceID, shape string, ocpus, memoryInGBs *float32, displayName string) (*core.Instance, error) {
